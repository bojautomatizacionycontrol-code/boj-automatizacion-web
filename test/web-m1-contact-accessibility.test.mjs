...
test("campos y errores tienen semántica accesible y autocompletado", () => {
  assert.ok((appSource.match(/autoComplete="name"/g) || []).length >= 4);
  assert.ok((appSource.match(/autoComplete="organization"/g) || []).length >= 3);
  assert.ok((appSource.match(/autoComplete="email"/g) || []).length >= 4);
  ...
  assert.ok((appSource.match(/aria-invalid=\{Boolean\(errors\./g) || []).length >= 13);
  assert.ok((appSource.match(/aria-describedby=\{errors\./g) || []).length >= 13);
...
test("cada formulario ofrece un enlace neutral de privacidad", () => {
  assert.equal((appSource.match(/className="form-privacy-note"/g) || []).length, 4);
  assert.equal((appSource.match(/href="\/privacidad"/g) || []).length >= 4, true);
